...
def cadastro_prod():
    ...
    inserir = "INSERT INTO cd_prod (id, nome, decricao , marca, tamanho, preco, preco_compra ) values (null, %s, %s, %s, %s, %s, %s)"
...
def alteracao_prod(): 
    ...
    consulta_sql= "SELECT * FROM bjmeire.cd_prod where nome= '{}'".format(nome_prod)
    cursor = connect.cursor()
...
def psq_func(): 
    ...
    consulta_sql= "SELECT * FROM bjmeire.cd_func where nome= '{}'".format(nome_psq)
    cursor = connect.cursor()
    cursor.execute(consulta_sql)
    linhas= cursor.fetchall() 
    cd_func.hide()
...
vendas=uic.loadUi('tela_venda.ui') # Inicializador para iniciar tela
...
cd_func.deletar.clicked.connect(confi_excluir_func)
